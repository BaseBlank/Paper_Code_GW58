...
from Vortex_Detection_tools import find_peaks, normalize, ASCII2D_Convert_matrix, direction_rotation, get_vortices

np.seterr(divide='ignore', invalid='ignore')

# re_results_folder = "F:\\model_generator\\No_Heating\\center_blockage_0.7\\2DArray"
re_results_folder = "C:\\Users\\1\\Desktop\\exampla"
result_files = os.listdir(re_results_folder)
...
# "second_order_diff" or "fourth_order_diff", second difference is enough
finite_difference_scheme = "second_order_diff"
detection_method = 'Q'
...
for file in file_sorted:
    file_path = re_results_folder + '/' + file
    ...
    if y_coordinate_size * x_coordinate_size == dx_tmp.shape[0]:
        ...
        if finite_difference_scheme == 'second_order_diff':
        ...
        if finite_difference_scheme == 'fourth_order_diff':
            dx = x_coordinate_step  # only for homogeneous mesh
            dy = y_coordinate_step  # only for homogeneous mesh
        ...
        if normalization_flag:
            print('Normalization for ', normalization_direction, ' direction')
            detection_field = normalize(detection_field, normalization_direction)  # normalization

        # ---- peak detection ---- #
        # 局部的最大值（峰值）并不总是与漩涡的中心相匹配,有时会偏离。
        print('PEAK DETECTION. Threshold={}, box size={}'.format(detection_threshold, box_size))
        peaks = find_peaks(detection_field, detection_threshold, box_size)  # Q criterion threshold
        print('Vortices found: ', len(peaks[0]))

        # ---- peaks direction of rotation ---- #
        vortices_clockwise, vortices_counterclockwise = direction_rotation(vorticity, peaks)

        # ---- model fitting ---- #
        vortices = list()
        if (plot_method == 'fit') and (xy_location == [0, 0]):
            vortices = get_vortices(x_coordinate_matrix, y_coordinate_matrix,
