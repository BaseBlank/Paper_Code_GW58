...
class CA_Block(nn.Module):
    ...
    def forward(self, x):
        _, _, h, w = x.size()  # [batch, C, H, W]

        # average pooling
        x_h = torch.mean(x, dim=3, keepdim=True).permute(0, 1, 3, 2)  # [batch, C, H, W] -> [batch, C, H, 1] -> [batch, C, 1, H]
        x_w = torch.mean(x, dim=2, keepdim=True)  # [batch, C, H, W] -> [batch, C, 1, W]

        # [batch, C, 1, H+W] -> [batch, C/r, 1, H+W]
        x_cat_conv_relu = self.relu(self.bn(self.conv_1x1(torch.cat((x_h, x_w), 3))))

        # [batch, C/r, 1, H+W] -> [batch, C/r, 1, H], [batch, C/r, 1, W]
        x_cat_conv_split_h, x_cat_conv_split_w = x_cat_conv_relu.split([h, w], 3)

        s_h = self.sigmoid_h(self.F_h(x_cat_conv_split_h.permute(0, 1, 3, 2)))  # [batch, C/r, 1, H] -> [batch, C, H, 1]
        s_w = self.sigmoid_w(self.F_w(x_cat_conv_split_w))  # [batch, C, 1, W]

        out = x * s_h.expand_as(x) * s_w.expand_as(x)  # [batch, C, H, W]
        return out
...
class RDN(nn.Module):
    def __init__(
        ...
        # Coord Attention(CA)
        # reduction, Reduce the amount of parameters for the entire attention mechanism
        self.ca = CA_Block(channels*num_rdb, reduction=2)
